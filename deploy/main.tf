...
resource "null_resource" "build_app" {
  ...
  provisioner "local-exec" {
    ...
    command = <<EOT
      ...
      docker build --file Dockerfile --tag=${var.app_name} .
      docker tag ${var.app_name} ${azurerm_container_registry.acr.login_server}/${var.app_name}
      docker push ${azurerm_container_registry.acr.login_server}/${var.app_name}
    EOT
    working_dir = "/home/dailymail"
  }
}

# Create the container and group
